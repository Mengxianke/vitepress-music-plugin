...
const mutations = {
    // payload为music数组
    updateMusicCollections(state, payload){
        state.musicCollections = payload;
        if (state.musicCollections.length > 0) {
            state.currentSongId = state.musicCollections[0].id;
        }
    },
    playNext (state) {
...
const actions = {
    updateMusicCollections({ commit }, params) {
        commit('updateMusicCollections', params.collections);
    },
